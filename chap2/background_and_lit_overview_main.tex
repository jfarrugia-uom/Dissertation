...
especially \textit{Facebook}} \\
    ...
    \bottomrule
    \end{tabular}
    %\caption{Hearst patterns and example phrases.}
    \caption[Hearst patterns and example phrases]{Hearst patterns and example phrases, with hypernym word annotated in bold.}
    \label{tab:hearst_pattern_phrase}
\end{table*}
...
Unsupervised methods consists of metrics grouped into four categories, each representative of a hypernymy inflected semantic hypothesis.  \citet{shwartz2017siege} furnish us with an exhaustive survey of metrics across all categories which are listed below.  
%In all descriptions, $\textbf{x}$ and $\textbf{y}$ represent the term word (hyponym) and candidate hypernym feature vector respectively.  
A summary of unsupervised measures which can be used to estimate hypernymy similarity can be seen in Table~\ref{tab:unsupervised_measures}.  

The notation employed in the formulae is slightly different from what is presented in \citet{shwartz2017siege}, and is intended to be consistent with the rest of the dissertation.  $x$ and $y$ represent the hyponym and hypernym words respectively; $\textbf{x}$, $\textbf{y}$ their vector equivalent.  $c$ is a particular word context feature represented by a vector index, and $x_c$ and $y_c$ are the scalar feature weights for the vectors $\textbf{x}$ and $\textbf{y}$ at the index pertaining to $c$.  In equations $APSyn$ and $APinc$, $rank_x(c)$ is the rank of context c in the sorted vector $\textbf{x}$.  $N(\textbf{x})$ referred to in $APSyn$ represents the set of $N$ (a hyperparameter) highest-ranked contexts of the sorted vector $\textbf{x}$, whereas $N_y$ ($APinc$) is the count of non-zero-valued contexts of $\textbf{y}$.  The $SLQS$ family depend on the entropy of a context $c$ in a word $x$, represented by the expression $H(x_c)$.
...
\begin{table*}\centering
    ...
    \begin{cases}
    1-\frac{rank_y\left(c_r\right)}{N_y + 1},& \text{if } c\in \textbf{y}\\
    0,              & \text{else}
    \end{cases}\right]}{N_y}$ & 
    ...
    $SLQS$                &
    $1-\displaystyle\frac{median_{i=1}^{N_x}\left( H\left(x_{c_i} \right)\right)}{median_{i=1}^{N_y}\left( H\left(y_{c_i} \right)\right)}$ & 
    \citep{santus2014chasing} \\[2ex]
    $SLQS_{Sub}$            &
    $med_{i=1}^{N_y}\left( H\left(y_{c_i} \right)\right) - med_{i=1}^{N_x}\left( H\left(x_{c_i} \right)\right)$ & 
    \citep{shwartz2017siege} \\[2ex]
    %$SLQS_{Row}$            && \citep{shwartz2017siege} \\[2ex]
    \cmidrule{1-3}
...
The distributional inclusional hypothesis is directly challenged by the reversed inclusional measures.  The basis of these measures is the hypernym cannot always replace the hyponym in its contexts due to the high specificity of the latter.  \textit{The mammal climbed into the car} would not be a reasonable alternative for \textit{the man climbed into car} despite the fact that a man is a mammal.  \citep{shwartz2017siege} propose a simple variant of Weeds Precision ($WeedsPrec$) and ClarkeDE ($CDE$) to model this property, consisting of switching the original function parameters around.  Thus $WeedsPrec(x,y)$ becomes $ReversedWeedsPrec(y, x)$ and the same principle applies to ClarkeDE.
...
\begin{table*}\centering
    ...
    \textit{SLQS $\times$ Cosine}   & $0.59$ & $0.27$ & $0.35$ & $0.24$ \\
    \bottomrule
    \end{tabular}
    %\caption{Average precision of two unsupervised metrics when discriminating semantic relations.}
    \caption[Average precision of two unsupervised metrics when discriminating semantic relations]{Average precision of $WeedsPrec$ and $SLQS \times Cosine$ unsupervised metrics when discriminating semantic relations.}
    \label{tab:santus_comparison}
\end{table*}
...
\begin{table*}\centering
    ...
    Kotlerman, 2010 & 0.346 & 0.437 && 0.408 & 0.375 & \textbf{0.461} \\
    BLESS, 2011 & 0.960 & 0.960 && \textbf{0.665} & 0.637 & 0.197 \\
    Baroni, 2012 & 0.638 & 0.802  && 0.774 & 0.663 & \textbf{0.788} \\
    Turney, 2014 & 0.644 & 0.747 && \textbf{0.696} & 0.649 & 0.642 \\
    Levy, 2014 & 0.302 & 0.370 && \textbf{0.324} & \textbf{0.324} & 0.231 \\
    \bottomrule
    \end{tabular}
    %\caption[Supervised learning results suggesting lexical memorisation.]{Supervised learning results suggesting lexical memorisation in \citep{levy2015supervised}.}
    \caption[Supervised learning $F_1$ results suggesting lexical memorisation.]{Supervised learning $F_1$ results suggesting lexical memorisation in \citep{levy2015supervised} exhibited on 5 diverse datasets. The results obtained when training a classifier on randomly-split data using lexical only and a combination of lexical and contextual features are shown on the left-hand side of the table.  On the right, we are displaying the scores obtained when training a supervised classifier on a lexical-split dataset.  The results obtained when training the classifier on hypernym-only features and a Cosine unsupervised baseline were added for the sake of comparison.  The $F_1$ score emitted by the best-performing model fitted on lexical-split data is annotated in bold.}
    \label{tab:levy_lexical_memo}
...
\begin{table*}\centering
    ...
    MRR & 0.215 & 0.279 & 0.321 & 0.343 && 0.193 & 0.349 & 0.280 & 0.339 \\
    \bottomrule
    \end{tabular}
    \caption{MRR comparison between pipeline and joint models and cluster size in \citep{yamane2016distributional}.}\label{tab:yamane_results}
\end{table*}
...
\begin{table*}\centering
    ...
    \bottomrule
    \end{tabular}
    %\caption{CRIM results achieved using projection learning only and hybrid results on the general-purpose English dataset \citep{bernier2018crim}.}
    \caption[Original CRIM results achieved using projection learning only and hybrid results on the general-purpose English dataset]{Original CRIM results achieved using projection learning only and hybrid results on the general-purpose English dataset in \citep{bernier2018crim}.  The best results, annotated in bold, were achieved by a supervised/Hearst-pattern hybrid model trained on an augmented dataset.}
    \label{tab:CRIM_results}
\end{table*}
...
\subsection{Evaluation Metrics} \label{meth_eval_metrics}
