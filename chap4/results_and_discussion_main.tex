...
We conclude this section, by selecting the best-performing configuration of each respective model: \ac{MSE}, CRIM and Yamane.  We ranked every model based on the geometric mean of the cross-validated \ac{MRR} and \ac{MAP} scores.  The hyperparameter combinations that led to the best model, based on our definition are shown in Table~\ref{tab:all_model_best_hyper} and the scores - together with standard error bars - can be seen in Figure~\ref{fig:part1_best_models}.




Our experiments did not exhaust all reasonable model configurations, so these results are by no means conclusive.  In the next section we shift focus to the SemEval-2018 Task9 English, general purpose task and the application of the knowledge gathered from these experiments to a "real life" problem setting.