...
\begin{table*}\centering
    \begin{tabular}{@{}lrrrr@{}} \toprule
    \multicolumn{5}{c}{\textbf{General-purpose English Dataset 1A}} \\
    & Term Count & Word-pairs & Distinct Hypernyms & Distinct Across Sets \\ \midrule
...
\end{table*}
As can be seen from Table~\ref{tab:semeval_english_dataset_metrics}, term words often feature a multitude of possible hypernyms.  Although the term words are lexically split in the training and test sets, the same cannot be said of the hypernyms.  Indeed, from 3,076 distinct hypernyms in the test set, 1,753 also feature in the $y$-slot of one or more word-pairs in the training set.  The lexical memorisation phenomenon studied in \citep{levy2015supervised} was also observed in one of the shared task's submitted work \citep{bernier2018crim}.  In the supervised context, creating lexically separated datasets would likely inhibit prototypical hypernyms from being "discovered" but would also reduce the training set appreciably, thus limiting the potential of a supervised model.

In addition to the training and test sets, a small validation set made up of 200 word-pairs (50 terms) was also provided.
