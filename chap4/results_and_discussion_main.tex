...
\bottomrule
\end{tabular}
%\caption{Regularisation effect on CRIM MAP and MRR results.}
...
%The overall best \ac{MAP} and \ac{MRR} are also in bold.  
...
\begin{table*}\centering
    ...
    \bottomrule
    \end{tabular}
    %\caption{Yamane hyperparameter settings and 5-fold cross-validated scores.}
...
\begin{table*}\centering
    ...
    \bottomrule
    \end{tabular}
    %\caption{Best-performing models' hyperparameter settings.}
...
\bottomrule
\end{tabular}
%\caption{Test results for models trained on concepts, entities, concepts and entities.}
...
\bottomrule
\end{tabular}
%\caption{Test results for best MTL models.}
...
\begin{figure}[!ht]
    \centering
    \subbottom[MRR]{\includegraphics[width=0.8\textwidth]{images/semeval_mrr_tuned.png} }\qquad
    \subbottom[MAP]{\includegraphics[width=0.8\textwidth]{images/semeval_map_tuned.png}}%
...
%On the other hand, we found that cluster size has the opposite effect on fastText features: MAP decreased from 0.34 when trained on a single cluster to 0.29 when trained on 25 clusters.  
...
In the second experimental objective, we applied the multi-projection model on the SemEval-2018 Task 9 shared task \citep{camacho2018semeval}.  In general, training on both word types helps discover hypernyms for unseen concepts, entities with frozen fastText embeddings coaxing the overall best scores of 0.267 and 0.133 for \ac{MRR} and \ac{MAP} respectively.  Two observations made in the first set of experiments hold here too: i) fastText features perform better than word2vec a d GloVe; and ii) with fastText, increasing the projections leads to better results.  Finally, tuning the embeddings in a second phase after separately learning the projection matrices boosted MRR and MAP by 30.35\% and 30\% respectively to 0.348 and 0.173.